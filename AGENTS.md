...
메모 CRUD 기능을 제공하는 Next.js 웹 애플리케이션. Supabase Postgres를 사용하여 데이터를 영속화하며, 카테고리 분류, 검색, AI 요약(Gemini) 기능을 지원한다.
...
- **Persistence:** Supabase Postgres (`@supabase/supabase-js` 클라이언트 직접 접근)
...
2. LocalStorage 접근 시 SSR 환경 체크 (`typeof window === 'undefined'`) 필수 (마이그레이션 보조 로직에서만 사용)
3. 메모 ID 생성은 uuid v4만 사용
4. TypeScript strict mode 위반 금지
5. Supabase 접근은 반드시 `supabaseMemos` 유틸 또는 `supabaseClient` 싱글톤을 통해서만 수행
...
- **[유틸리티 (Utils)](./src/utils/AGENTS.md)** — 헬퍼 함수 및 Supabase CRUD 로직 작업 시
